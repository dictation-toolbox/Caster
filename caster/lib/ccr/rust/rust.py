...
class RustNon(MappingRule):
    mapping = {
        ...
        "ternary":                    R(Text("if TOKEN == TOKEN { TOKEN } else { TOKEN }"), rdescript="Rust: Ternary"),
        "function [<return>]":        R(Text("fn TOKEN(TOKEN)%(return)s{}"),  rdescript="Rust: Function"),
...
class Rust(MergeRule):
    ...
    mapping = {
        ...
        #
        SymbolSpecs.DO_LOOP:            R(Text("while {TOKEN;TOKEN}{}"), rdescript="Rust: Do Loop"),
        SymbolSpecs.WHILE_LOOP:         R(Text("while TOKEN {}")+Key("left"), rdescript="Rust: While"),
        "for loop [of <a> [in <n>]]":   R(Text("for %(a)s in 0..%(n)d {}")+Key("left"), rdescript="Rust: For i Loop"),
        SymbolSpecs.FOR_EACH_LOOP:      R(Text("for TOKEN in TOKEN {}")+Key("left"), rdescript="Rust: For Each Loop"), 
        #
        ...
        "enumerate for loop [of <a> [in <n>]]": R(Text("for (%(a)s, TOKEN) in (0..%(n)d).enumerate() {}")+Key("left"), rdescript="Rust: Enumerated For i Loop"),
        "enumerate for each [<a> <b>]":         R(Text("for (%(a)s, %(b)s) in TOKEN.enumerate() {}")+Key("left"), rdescript="Rust: Enumerated For Each Loop"),
        ...
        "array [of] size <n>":          R(Text("[TOKEN; %(n)d]"), rdescript="Rust: Array"), 
