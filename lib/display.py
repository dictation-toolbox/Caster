...
class LegionGrid(TkTransparent):
    class LServer(BottleServer):
        def __init__(self):
            '''
            Problems: 
            1. tk needs to be using the same block as the server: pass a lock into the server from tk
            2. Having tk use a timer to check if an update is available introduces latency: use a callback instead, passed down into the server
