...
class TyporaRule(MergeRule):
    ...
    mapping = {
        ...
        "new line <h>":
            R(Key("s-enter")) * Repeat(extra="h"),
        "copy [as] markdown":
            R(Key("cs-c")),
        "delete row <n>":
            R(Key("cs-backspace")) * Repeat(extra="n"),
        "select [cell | scope]":
            R(Key("c-e")),
        "[select] word <n>":
            R(Key("c-d")) * Repeat(extra="n"),
        "delete word <n>":
            R(Key("cs-d")) * Repeat(extra="n"),
        ...
        "increase heading [level] <h>":
            R(Key("c-equal")) * Repeat(extra="h"),
        "decrease heading [level] <h>":
            R(Key("c-minus")) * Repeat(extra="h"),
        "table":
            R(Key("c-t")),  # could be automated.
        "code fences":
            R(Key("cs-k")),
        "math block":
            R(Key("cs-m")),
