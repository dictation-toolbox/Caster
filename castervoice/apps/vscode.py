...
class VSCodeCcrRule(MergeRule):
    ...
    mapping = {
       # note: if you get the bad grammar grammar too complex error, move some of these commands into the non-CCR rule
        ...
        "cursor above [<n>]": R(Key("ca-up"), 
            rdescript="insert cursor above") * Repeat(extra='n'),
        "cursor below [<n>]": R(Key("ca-down"),
            rdescript="insert cursor above") * Repeat(extra='n'),
            
        "remove cursor": R(Key("csa-down"), rdescript=""), #not sure if this command works always; also try csa-up
        ...
        "next word [<n>]": R(Key("c-f3"),
            rdescript="select next occurrence of current word") * Repeat(extra='n'),
        "select next word [<n>]": R(Key("sa-right/2, c-f3, c-left/2, escape"), 
        ...
        "select prior word [<n>]": R(Key("cs-f3"),
            rdescript="select prior occurrence of current word") * Repeat(extra='n'),
        "prior word [<n>]": R(Key("sa-right/2, cs-f3, c-left/2, escape"),
        ...
        "[select] between prekris [<n>]": R(Key("c-k, lparen"), rdescript="select between parentheses noninclusive using 'quick and simple text selection' VScode extension") * Repeat(extra='n'),
        "[select] around prekris [<n>]": R(Key("c-k, rparen"), rdescript="select between parentheses inclusive using 'quick and simple text selection' VScode extension") * Repeat(extra='n'),
        "[select] between single [<n>]": R(Key("c-k, squote"), rdescript="select between single quotes noninclusive using 'quick and simple text selection' VScode extensios") * Repeat(extra='n'),
        "[select] between quotes [<n>]": R(Key("c-k, dquote"), rdescript="select between double quotes noninclusive using 'quick and simple text selection' VScode extension")* Repeat(extra='n'),
        "[select] between brax [<n>]": R(Key("c-k, lbracket"), rdescript="select between square brackets noninclusive using 'quick and simple text selection' VScode extension")* Repeat(extra='n'),
        "[select] around brax [<n>]": R(Key("c-k, rbracket"), rdescript="select between square brackets inclusive using 'quick and simple text selection' VScode extension")* Repeat(extra='n'),
        "[select] between curly [<n>]": R(Key("c-k, lbrace"), rdescript="select between curly brace noninclusive using 'quick and simple text selection' VScode extension")* Repeat(extra='n'),
        "[select] around curly [<n>]": R(Key("c-k, rbrace"), rdescript="select between curly brace inclusive using 'quick and simple text selection' VScode extension")* Repeat(extra='n'),
        "[select] between angle [<n>]": R(Key("c-k, langle"), rdescript="select between angle brackets noninclusive using 'quick and simple text selection' VScode extension")* Repeat(extra='n'),
...
class VisualStudioCodeNonCcrRule(MergeRule):
    ...
    mapping = {
        ...
        "toggle orientation": R(Key("sa-0"), rdescript="toggle orientation"),
        "zoom in [<n>]": R(Key("c-equal"), rdescript="zoom in") * Repeat(extra='n'),
        "zoom out [<n>]": R(Key("c-minus"), rdescript="zoom out") * Repeat(extra='n'),
        ...
        "new window": R(Key("cs-n"), rdescript="new window"),
        "close window": R(Key("a-f4"), rdescript="close window"),
        "close workspace": R(Key("c-k, f"), rdescript="close workspace"),
        "close editor": R(Key("c-f4"), rdescript="close editor"),
        "save as": R(Key("cs-s"), rdescript="save as"),
        "save all": R(Key("c-k, s"), rdescript="Save all"),
        "nexta [<n>]":
        ...
        "(recent | R) tab [<n>]": R(Key("c-tab"),  
            rdescript="go to most recent tab") * Repeat(extra='n'),
        "reopen tab [<n>]": R(Key("cs-t"), 
            rdescript="reopen most recently tab") * Repeat(extra='n'),
        "Exit preview":
            R(Key("space, c-z"), rdescript="Visual Studio Code: Exit Preview"),
        "keep preview mode editor open": R(Key("c-k, enter"), 
            rdescript="keep preview mode editor open"),
        "copy path": R(Key("c-k, p"), rdescript="copy path of active file"),
        "reveal active file in Explorer": R(Key("c-k, r"), 
            rdescript="open Windows Explorer to the location of active file"),
        "show active file in new window": R(Key("c-k, o"), 
            rdescript="show active file in new window"),
        ...
        "show all symbols": R(Key("c-t"), rdescript="show all symbols"),
        "go to symbol": R(Key("cs-o"), rdescript="go to symbol"),

              
        # editor management
        "close editor": R(Key("c-w"), rdescript="close editor"),
        "close folder": R(Key("c-k, f"), rdescript="close folder"),
        "split editor": R(Key("c-backslash"), rdescript="split editor into 2 panes"),
        "next pane": R(Key("c-k, c-right"), rdescript="move to next pane"),
        ...
        "peak definition": R(Key("a-f12"), rdescript="peak definition"),
        "trigger parameter hints": R(Key("cs-space"), rdescript="trigger parameter hints"),
        "format that": R(Key("c-k, c-f"), rdescript="format selection"),
        "(definition to side | side def)": R(Key("c-k, f12"), 
             rdescript="open definition to the side"), 
        "show references": R(Key("s-f12"), rdescript="show references"),
        "rename symbol": R(Key("f2"), rdescript="rename symbol"),
        "(trim white)": R(Key("c-k, c-x"), 
            rdescript="trim trailing white space"),
        "change file language": R(Key("c-k, m"), rdescript="change file language"),

            
                            
               
        # Debugging
        "debug": R(Key("cs-d"), rdescript="debug"),
        "[toggle] breakpoint":
        ...
        "stopper": R(Key("s-f5"), rdescript="debug stop"),
        "continue": R(Key("f5"), rdescript="start/continue"),
        "show hover": R(Key("c-k, c-i"), 
        ...
        "[show] problems [panel]": R(Key("cs-m"), rdescript="show problems panel"),
        "next error": R(Key("f8"), rdescript="go to next error"), # doesn't seem to be working properly
        "(prior | previous) error": R(Key("s-f8"), rdescript="go to previous error"),
        "toggle tab moves focus": R(Key("c-m"), rdescript="toggle taboos focus"),
            

        # find and replace (see also multi-cursor and selection section above)
        "replace": R(Key("c-h"), rdescript="replace"),
        "find in files": R(Key("cs-f"), rdescript="find in files"),
        "replace in files": R(Key("cs-h"), rdescript="replace in files"),
        "next find": R(Key("f3"), rdescript="go to next occurrence"),
        "(prior | previous) find": R(Key("s-f3"), rdescript="go to previous occurrence"),
        ...
        "toggle case sensitive": R(Key("a-c"), rdescript="toggle case-sensitive"),
        "toggle regex": R(Key("a-r"), rdescript="toggle regular expressions"),
        "toggle whole word": R(Key("a-w"), rdescript="toggle whole word"),


        # integrated terminal
        "[show] terminal": R(Key("c-backtick"), rdescript="show integrated terminal"),
        "new terminal": R(Key("cs-backtick"), rdescript="new integrated terminal"),
        "terminal scroll up": R(Key("c-up"), rdescript="terminal scroll up"),
        "terminal scroll down": R(Key("c-down"), rdescript="terminal scroll down"),
        "terminal page up": R(Key("s-pgup"), rdescript="terminal page up"),
        "terminal page down": R(Key("s-pgdown"), rdescript="terminal page down"),

        
        # collapsing
        "(fold | collapse) region": R(Key("cs-lbracket"), rdescript="collapse region"),
        "(unfold | uncollapse) region": R(Key("cs-rbracket"), rdescript="uncollapse region"),
        "(fold | collapse) [all] subregions": R(Key("c-k, c-lbracket"),
            rdescript="collapse all subregions"),
        "(unfold | uncollapse) [all] subregions": R(Key("c-k, c-rbracket"),
             rdescript="uncollapse all subregions"),
        "(fold | collapse) [all] regions": R(Key("c-k, c-0"),
             rdescript="safe collapse all regions"),
        "(unfold | uncollapse) [all] regions": R(Key("c-k, c-j"),
             rdescript="on collapse all regions"),
        ...
        "toggle blame": R(Key("cs-g, b"), rdescript="toggle blame"),
        "lens commit details": R(Key("cs-g, c"), rdescript="lens commit details"),
        "lens file history": R(Key("cs-g, h"), rdescript="lens file history"),
        "lens repo status": R(Key("cs-g, s"), rdescript="lens repo status"),
        "toggle git lens": R(Key("cs-g, s-b"), rdescript="toggle git lens"),

        # requires bookmark extension
        "mark (prev | prior | previous)": R(Key("ca-j"), rdescript="Mark previous"),
        "mark next": R(Key("ca-l"), rdescript="Mark next"),

        # requires the keyboard-scroll extension (once=middle, twice=top, thrice=bottom)
        "center mass": R(Key("c-l"), rdescript="center mass"),
